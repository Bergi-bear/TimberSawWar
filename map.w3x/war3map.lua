...
function CreateUnitsForPlayer10()
    ...
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n002"), -5989.9, 5816.3, 83.977)
    u = CreateUnit(p, FourCC("n001"), -5798.5, 4603.5, 337.950)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5938.4, 4691.2, 118.394)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5439.8, 4712.9, 61.492)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5518.0, 4457.1, 53.219)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5440.4, 4574.9, 75.193)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5509.9, 4638.8, 133.103)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5590.4, 4724.8, 5.801)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5672.7, 4638.8, 66.755)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5699.1, 4548.8, 322.250)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5681.2, 4485.2, 129.599)
    SetUnitAcquireRange(u, 200.0)
    u = CreateUnit(p, FourCC("n001"), -5593.1, 4470.9, 289.949)
    SetUnitAcquireRange(u, 200.0)
...
function InitGameCore()
	for i = 0, bj_MAX_PLAYER_SLOTS - 1 do
		...
		if i==0 then -- GetPlayerController(player) == MAP_CONTROL_USER and GetPlayerSlotState(player) == PLAYER_SLOT_STATE_PLAYING then
			...
			--таблицы
			Talants[pid]={
				--main=CreateUnit(player, FourCC('e009'), -0, 0, 0),
				--q=CreateUnit(player, FourCC('e001'), -0, 0, 0),
				w=CreateUnit(player, FourCC('e004'), -0, 0, 0)
				--e=CreateUnit(player, FourCC('e001'), -0, 0, 0),
				--r=CreateUnit(player, FourCC('e001'), -0, 0, 0)
...
function InitTalants()
		...
		TimerStart(timer, 0.0, false, function()
			...
			TriggerAddAction(UI_SHOP_TRIGGER, function()
				...
				SetUnitPosition(data.w,GetUnitX(hero),GetUnitY(hero))
				UnitRemoveAbility(data.w,FourCC('Aloc'))
				SelectUnitForPlayerSingle(data.w,p)
				print(GetUnitName(data.w))
